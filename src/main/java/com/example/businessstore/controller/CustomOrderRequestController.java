...
public class CustomOrderRequestController {
    ...
    @PostMapping public ResponseEntity<ApiResponse<CustomOrderRequestResponse>> create(@AuthenticationPrincipal Jwt jwt, @Valid @RequestBody CreateCustomOrderRequest request) { return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.success(service.create(userId(jwt), request), "Custom order request created")); }
