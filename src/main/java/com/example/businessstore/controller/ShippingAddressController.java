...
public class ShippingAddressController {
    ...
    @PostMapping public ResponseEntity<ApiResponse<ShippingAddressResponse>> create(@AuthenticationPrincipal Jwt jwt, @Valid @RequestBody CreateShippingAddressRequest request) { return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.success(service.create(userId(jwt), request), "Đã thêm địa chỉ giao hàng.")); }
    ...
    @PutMapping("/{id}") public ResponseEntity<ApiResponse<ShippingAddressResponse>> update(@AuthenticationPrincipal Jwt jwt, @PathVariable UUID id, @Valid @RequestBody UpdateShippingAddressRequest request) { return ResponseEntity.ok(ApiResponse.success(service.update(userId(jwt), id, request), "Đã cập nhật địa chỉ giao hàng.")); }
