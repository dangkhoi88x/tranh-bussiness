...
public class OrderController {
    ...
    @PostMapping("/checkout") public ResponseEntity<ApiResponse<OrderResponse>> checkout(@AuthenticationPrincipal Jwt jwt, @Valid @RequestBody CheckoutOrderRequest request) { return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.success(orderService.checkout(userId(jwt), request), "Đã tạo đơn hàng.")); }
    ...
    @PutMapping("/my-orders/{id}/cancel") public ResponseEntity<ApiResponse<OrderResponse>> cancel(@AuthenticationPrincipal Jwt jwt, @PathVariable UUID id) { return ResponseEntity.ok(ApiResponse.success(orderService.cancel(userId(jwt), id), "Đã huỷ đơn hàng.")); }
