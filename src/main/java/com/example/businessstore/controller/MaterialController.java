...
public class MaterialController {
    ...
    @PostMapping @PreAuthorize(SecurityExpressions.CAN_MANAGE_PRODUCTS) public ResponseEntity<ApiResponse<MaterialResponse>> create(@Valid @RequestBody CreateMaterialRequest request) { return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.success(materialService.create(request), "Material created")); }
    @PutMapping("/{id}") @PreAuthorize(SecurityExpressions.CAN_MANAGE_PRODUCTS) public ResponseEntity<ApiResponse<MaterialResponse>> update(@PathVariable UUID id, @Valid @RequestBody UpdateMaterialRequest request) { return ResponseEntity.ok(ApiResponse.success(materialService.update(id, request), "Material updated")); }
}
