...
public class ShippingAddressServiceImpl implements ShippingAddressService {
    ...
    @Override @Transactional(readOnly = true) public ShippingAddress getOwned(UUID userId, UUID id) { return addressRepository.findByIdAndUserId(id, userId).orElseThrow(() -> new AppException(ErrorCode.SHIPPING_ADDRESS_NOT_FOUND, "Không tìm thấy địa chỉ giao hàng.")); }
    private User user(UUID id) { return userRepository.findById(id).orElseThrow(() -> new AppException(ErrorCode.UNAUTHORIZED, "Không tìm thấy người dùng.")); }
