...
public class CustomOrderRequestServiceImpl implements CustomOrderRequestService {
    ...
    @Override @Transactional public CustomOrderRequestResponse uploadImage(UUID userId, UUID id, MultipartFile file) {
        CustomOrderRequest request = owned(id, userId); if (request.getStatus() != CustomOrderRequestStatus.NEW) throw new AppException(ErrorCode.CUSTOM_ORDER_REQUEST_NOT_EDITABLE, "Reference images can be added only before quoting");
    ...
    @Override @Transactional public CustomOrderRequestResponse decideQuote(UUID userId, UUID id, DecideCustomOrderQuoteRequest input) {
        ...
        if (request.getStatus() != CustomOrderRequestStatus.QUOTED) throw new AppException(ErrorCode.INVALID_CUSTOM_ORDER_STATUS, "Only quoted requests can be accepted or declined");
        if (!input.accepted()) { request.setStatus(CustomOrderRequestStatus.CANCELLED); return toResponse(request); }
        if (input.shippingAddressId() == null) throw new AppException(ErrorCode.CUSTOM_ORDER_SHIPPING_ADDRESS_REQUIRED, "A delivery address is required to accept a custom quote");
