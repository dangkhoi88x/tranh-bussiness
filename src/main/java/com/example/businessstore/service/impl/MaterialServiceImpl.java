...
public class MaterialServiceImpl implements MaterialService {
    ...
    public MaterialResponse create(CreateMaterialRequest request) {
        ...
        if (materialRepository.existsByCodeIgnoreCase(code)) throw new AppException(ErrorCode.MATERIAL_CODE_ALREADY_EXISTS, "Material code already exists");
    ...
    public MaterialResponse update(UUID id, UpdateMaterialRequest request) {
        ...
        if (request.code() != null) { String code = normalizeCode(request.code()); if (materialRepository.existsByCodeIgnoreCaseAndIdNot(code, id)) throw new AppException(ErrorCode.MATERIAL_CODE_ALREADY_EXISTS, "Material code already exists"); material.setCode(code); }
    ...
    @Override @Transactional(readOnly = true)
    public Material requireActiveArtworkSurface(UUID materialId) { return materialRepository.findByIdAndScopeAndStatus(materialId, MaterialScope.ARTWORK_SURFACE, MaterialStatus.ACTIVE).orElseThrow(() -> new AppException(ErrorCode.MATERIAL_NOT_ACTIVE, "Select an active artwork material")); }

    private Material get(UUID id) { return materialRepository.findById(id).orElseThrow(() -> new AppException(ErrorCode.MATERIAL_NOT_FOUND, "Material not found")); }
    ...
    private String normalizeCode(String value) { String code = value.trim().toUpperCase(Locale.ROOT).replaceAll("[^A-Z0-9]+", "_").replaceAll("^_+|_+$", ""); if (code.isBlank()) throw new AppException(ErrorCode.INVALID_MATERIAL, "Material code must contain letters or numbers"); return code; }
