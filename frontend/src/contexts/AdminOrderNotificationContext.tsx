...
export function AdminOrderNotificationProvider({ children }: { children: ReactNode }) {
  ...
  const receiveOrder = useCallback(
    (order: AdminNewOrderNotification) => {
  ...
  const value = useMemo(
    () => ({ newOrderCount, latestOrder, soundEnabled, enableSound, clearNewOrders }),
    [clearNewOrders, enableSound, latestOrder, newOrderCount, soundEnabled],
  );
