...
export function BestSellers({ categoryId, intervalMs = 3000 }: { categoryId?: string; intervalMs?: number }) {
  ...
  return (
    ...
    >
      ...
      >
        <span
          style={{ fontSize: 11, letterSpacing: '.18em', textTransform: 'uppercase', color: 'var(--color-accent-700)' }}
        >
          02/ tranh in canvas
        </span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 'var(--space-4)' }}>
          ...
          <a href="/danh-muc/tranh-canvas" className="btn btn-secondary" style={{ whiteSpace: 'nowrap' }}>
            Xem thêm
          </a>
        </div>
      </div>
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
          <button type="button" aria-label="Tranh trước" style={navBtn} onClick={() => step(1)}>
            ←
          </button>
          <button type="button" aria-label="Tranh sau" style={navBtn} onClick={() => step(-1)}>
            →
          </button>
        </div>
      </div>
