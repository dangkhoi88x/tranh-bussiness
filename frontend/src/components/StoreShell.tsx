...
export function cartItemOptions(item: CartItem): string {
  ...
  return [size, variant?.material, item.selectedFrameOption?.frameName ?? 'Căng viền']
    .filter(Boolean).join(' · ');
}

export function StoreSummaryRow({ label, value, valueTone }: {
  label: string;
  value: string;
...
export function StoreNotice({ title, body, action }: {
  title: string;
  body: string;
  action?: ReactNode;
}) {
  return (
    <section style={{
      ...
      <h2 style={{
        ...
        fontSize: 26, letterSpacing: '-.025em',
      }}>{title}</h2>
      {body && (
        <p style={{
          margin: 0, maxWidth: '46ch', fontSize: 15, lineHeight: 1.6,
