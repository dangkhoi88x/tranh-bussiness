...
export function ViewInRoom({ image }: { image?: string }) {
  return (
    <section id="uom-tranh" style={{ borderTop: '2px solid var(--color-text)', background: 'var(--color-bg)' }}>
      <div
        data-split=""
        style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.25fr) minmax(0, 1fr)', alignItems: 'stretch' }}
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', padding: 'var(--space-8)' }}>
          ...
          <p style={{ margin: 0, maxWidth: '46ch', fontSize: 15, lineHeight: 1.6, color: 'var(--color-neutral-800)' }}>
            Chưa chắc nên chọn khổ nào? Chụp một tấm ảnh bức tường, chọn tranh và đổi qua lại giữa các khổ ngay trên ảnh
            đó cho tới khi vừa mắt.
          </p>
          ...
          <a href="/thu-tren-tuong" className="btn btn-primary btn-block" style={{ marginTop: 'auto' }}>
            Thử ngay
          </a>
        </div>
      </div>
