  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
      ...
      >
        {fields.map(({ key, label, placeholder }) => (
          <div key={key} className="field">
            ...
            />
            {fieldErrors[key] && (
              <span style={{ display: 'block', marginTop: 2, fontSize: 11, color: 'var(--color-accent-700)' }}>
                {fieldErrors[key]}
              </span>
            )}
          </div>
      ...
      <label style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', fontSize: 13, cursor: 'pointer' }}>
        <input
          type="checkbox"
          checked={Boolean(form.defaultAddress)}
          onChange={(event) => setField('defaultAddress', event.target.checked)}
        />
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-3)' }}>
        <button type="button" className="btn btn-primary" disabled={saving} onClick={onSave}>
          {saving ? 'Đang lưu…' : (submitLabel ?? 'Lưu địa chỉ')}
        </button>
        {onCancel && (
