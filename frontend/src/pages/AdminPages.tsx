...
type DashboardData = {
  ...
  lowStockProducts: { id: string; name: string; categoryName: string; stockQuantity: number }[]
}
  ...
  return <>
    ...
    {loading && !data ? <section className="catalog-panel"><p className="table-loading">Đang tổng hợp số liệu…</p></section> : data && <>
      <section className="dashboard-metrics">
        ...
        <MetricCard label="Sắp hết hàng" value={String(data.lowStockCount)} note={`Tồn kho ≤ ${data.lowStockThreshold} sản phẩm`} tone="danger" link="/admin/products" />
      </section>
      <section className="dashboard-grid">
        ...
        <section className="catalog-panel dashboard-card dashboard-card--wide"><header className="panel-heading"><div><h3>Sản phẩm cần kiểm tra tồn kho</h3><p>Chỉ hiển thị sản phẩm đang PUBLISHED có tồn kho không quá {data.lowStockThreshold}.</p></div><Link className="dashboard-link" to="/admin/products">Quản lý sản phẩm</Link></header>{data.lowStockProducts.length > 0 ? <table className="data-table"><thead><tr><th>Sản phẩm</th><th>Danh mục</th><th>Tồn kho</th><th /></tr></thead><tbody>{data.lowStockProducts.map((product) => <tr key={product.id}><td><strong>{product.name}</strong></td><td>{product.categoryName}</td><td><span className={`stock-pill${product.stockQuantity === 0 ? ' stock-pill--empty' : ''}`}>{product.stockQuantity}</span></td><td><Link className="dashboard-link" to={`/admin/products/${product.id}`}>Mở chi tiết</Link></td></tr>)}</tbody></table> : <p className="empty-state">Không có sản phẩm PUBLISHED nào sắp hết hàng.</p>}</section>
      </section>
    </>}
  </>
}
