...
export function AuthPage() {
  ...
  async function handleForgotSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setNotice({
        kind: 'success',
        text: `Nếu ${email} có tài khoản, chúng tôi vừa gửi liên kết đặt lại mật khẩu. Liên kết hết hạn sau 30 phút; nhớ xem cả hộp thư rác.`,
      });
    } catch (error) {
      setNotice({
        kind: 'error',
        text: error instanceof Error ? error.message : 'Không thể gửi liên kết đặt lại mật khẩu.',
      });
    } finally {
      setSubmitting(false);
    }
      ...
      <AuthShell>
        <section className="auth-card" aria-labelledby="auth-title">
          <header className="auth-card__header">
            ...
            <h2 id="auth-title">Quên mật khẩu</h2>
            <p>Nhập email bạn đã đăng ký. Chúng tôi sẽ gửi một liên kết để bạn đặt mật khẩu mới.</p>
          </header>
          {notice && (
          ...
          <form className="auth-form" onSubmit={(event) => void handleForgotSubmit(event)}>
            <label>
              Email
              <input type="email" name="email" autoComplete="email" placeholder="ban@example.com" required />
            </label>
  ...
  return (
    <AuthShell>
      <section className="auth-card" aria-labelledby="auth-title">
        <div className="mode-switch" role="tablist" aria-label="Chọn hình thức xác thực">
          <button
            type="button"
            role="tab"
        ...
        <header className="auth-card__header">
          <p className="eyebrow">{mode === 'login' ? 'CHÀO MỪNG BẠN TRỞ LẠI' : 'BẮT ĐẦU HÀNH TRÌNH'}</p>
          <h2 id="auth-title">{mode === 'login' ? 'Đăng nhập tài khoản' : 'Tạo tài khoản mới'}</h2>
          <p>
            {mode === 'login' ? 'Nhập thông tin của bạn để tiếp tục.' : 'Chỉ mất một phút để tham gia cùng chúng tôi.'}
          </p>
        </header>
        {notice && (
          <div className={`notice notice--${notice.kind === 'success' ? 'success' : 'error'}`} role="status">
        ...
        <form className="auth-form" onSubmit={(event) => void handleSubmit(event)}>
            ...
            <div className="form-row">
              <label>
                ...
                <input
                  ...
                  onChange={(event) => setRegistration({ ...registration, lastName: event.target.value })}
                  placeholder="Nguyễn"
                />
              </label>
              <label>
                ...
                <input
                  ...
                  onChange={(event) => setRegistration({ ...registration, firstName: event.target.value })}
                  placeholder="An"
                />
              </label>
            </div>
          ...
          {mode === 'register' && (
            <label>
              Số điện thoại <em>(không bắt buộc)</em>
              <input
                type="tel"
                autoComplete="tel"
          ...
          {mode === 'login' && (
            <button type="button" className="forgot-link" onClick={() => selectMode('forgot')}>
              Quên mật khẩu?
            </button>
          )}
          {mode === 'register' && (
            <label>
              ...
              <input
                ...
                onChange={(event) => setRegistration({ ...registration, confirmPassword: event.target.value })}
                placeholder="••••••••••••"
                required
              />
            </label>
          )}
          {mode === 'register' && <p className="password-hint">Tối thiểu 12 ký tự để bảo vệ tài khoản của bạn.</p>}
          <button className="primary-button" type="submit" disabled={submitting}>
        ...
        {isGoogleSignInAvailable() && (
          <>
            ...
            <button
              ...
              onClick={() => {
                window.location.href = beginGoogleSignIn();
              }}
            >
              <GoogleMark />
              Tiếp tục với Google
