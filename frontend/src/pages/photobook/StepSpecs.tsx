...
export function StepSpecs({
  ...
  pageIndex: number;
  setPageIndex: (fn: number | ((i: number) => number)) => void;
  pageOptions: { pageCount: number; price: number }[];
  selected: { pageCount: number; price: number } | null;
  finish: string;
  setFinish: (f: string) => void;
  qty: number;
  setQty: (fn: number | ((q: number) => number)) => void;
  price: number;
  photos: { min: number; max: number } | null;
  slotCapacity: number | null;
  photoCount: string;
  setPhotoCount: (v: string) => void;
  suggestion: number | null;
  templates: PhotobookTemplate[];
  templateId: string;
  setTemplateId: (id: string) => void;
  spreadsHaveImages: boolean;
  ...
  return (
    ...
    >
              ...
              <tbody>
                {(pricing.sizes[0]?.pageOptions ?? []).slice(0, 6).map((opt, idx) => (
                  <tr key={opt.pageCount}>
                    ...
                    {pricing.sizes.map((s) => (
                      <td key={s.variantId}>{formatPrice(s.pageOptions[idx]?.price)}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
      ...
      <div
        id="mua"
        data-pb-buy=""
        style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)', padding: 'var(--space-8)' }}
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
          ...
          {product.description && (
            <p
              style={{ margin: 0, maxWidth: '48ch', fontSize: 15, lineHeight: 1.6, color: 'var(--color-neutral-800)' }}
            >
              {product.description}
            </p>
          )}
              ...
              {pricing.sizes.map((s) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => setSizeId(s.variantId)}
                    style={chipStyle(on, !s.available)}
                  >
                    {s.name}
                  </button>
                );
          ...
          <div data-option-row="" style={optionRow}>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', border: '2px solid var(--color-text)' }}>
                <button
                  type="button"
                  aria-label="Bớt trang"
                  disabled={pageIndex <= 0}
                  onClick={() => setPageIndex((i: number) => Math.max(0, i - 1))}
                  style={stepBtnStyle(pageIndex <= 0)}
                >
                  −
                </button>
                ...
                <button
                  ...
                  style={stepBtnStyle(pageIndex >= pageOptions.length - 1)}
                >
                  +
                </button>
              </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
          <label htmlFor="photo-count" style={STORE_LABEL_STYLE}>
            Bạn có bao nhiêu ảnh?
          </label>
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (spreadsHaveImages && tpl.id !== templateId) setPendingApply(true);
                  }}
                  aria-pressed={on}
                  style={{
                    appearance: 'none',
          ...
          {pendingApply && (
            <div
              style={{
                ...
                flexWrap: 'wrap',
                padding: 'var(--space-3)',
                background: 'var(--color-accent-100, rgba(180,60,60,.08))',
                borderRadius: 4,
                fontSize: 13,
              }}
            >
        ...
        <button
          ...
          disabled={!selected || !size?.available}
          onClick={onNext}
