      ...
      >
        {spreads.map((spread, idx) => {
          ...
          return (
            <div
              key={idx}
              onDragOver={(event) => updateDropTarget(event, idx)}
              onDragLeave={(event) => {
                if (!event.currentTarget.contains(event.relatedTarget as Node))
                  setDragOver((current) => (current?.index === idx ? null : current));
              }}
              onDrop={(event) => dropOn(event, idx)}
              style={{
                position: 'relative',
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  gap: 4,
                  border: `2px solid ${activeIndex === idx ? 'var(--color-accent)' : 'var(--color-neutral-300)'}`,
                  borderRadius: 4,
              ...
              <button
                type="button"
                draggable
                onDragStart={(event) => beginDrag(event, idx)}
                onDragEnd={() => {
                  setDragFrom(null);
                  setDragOver(null);
                }}
                aria-label={`Kéo spread ${idx + 1} để đổi thứ tự`}
              ...
              <div
                aria-label={`Di chuyển spread ${idx + 1}`}
                style={{
                  position: 'absolute',
                  zIndex: 3,
                  right: compact ? 6 : 8,
                  bottom: compact ? 24 : 27,
                  display: 'flex',
                  gap: 3,
                }}
              >
                <button
                  ...
                  aria-label={`Đưa spread ${idx + 1} lên trước`}
                ...
                <button
                  ...
                  onClick={() => onMove(idx, idx + 1)}
                  aria-label={`Đưa spread ${idx + 1} xuống sau`}
                  style={moveButtonStyle}
                >
                  ↓
                </button>
