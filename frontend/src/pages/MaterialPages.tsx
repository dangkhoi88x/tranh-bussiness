...
export function MaterialsPage() {
  ...
  async function load() {
    try {
      setItems(await apiRequest<Material[]>(`/materials/management?scope=${scope}`));
    } catch (error) {
    ...
    <>
      <header className="catalog-header">
        <div>
          ...
          <h2>Chất liệu</h2>
          <p>Chuẩn hóa chất liệu để variant, bộ lọc và báo cáo luôn dùng cùng một dữ liệu.</p>
        </div>
      ...
      <div className="detail-tabs">
        <button className={scope === 'ARTWORK_SURFACE' ? 'is-active' : ''} onClick={() => setScope('ARTWORK_SURFACE')}>
          Bề mặt tranh
        </button>
        <button className={scope === 'FRAME' ? 'is-active' : ''} onClick={() => setScope('FRAME')}>
          Chất liệu khung
        </button>
      </div>
      <Panel>
              ...
              <tr key={item.id}>
                ...
                <td>
                  <span className={`status status--${item.status.toLowerCase()}`}>
                    {item.status === 'ACTIVE' ? 'Đang dùng' : 'Đã lưu trữ'}
                  </span>
                </td>
        ...
        {items.length === 0 && <p className="empty-state">Chưa có chất liệu trong nhóm này.</p>}
      </Panel>
      {editing !== undefined && (
        <MaterialForm
          item={editing}
...
function MaterialForm({
  ...
  async function submit(event: FormEvent) {
    ...
    setError(null);
    try {
      await apiRequest(item ? `/materials/${item.id}` : '/materials', {
        method: item ? 'PUT' : 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...form, scope, description: form.description || null }),
      });
      onSaved();
    } catch (cause) {
  ...
  return (
    <Modal title={item ? 'Sửa chất liệu' : 'Thêm chất liệu'} onClose={onClose}>
