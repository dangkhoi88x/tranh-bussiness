      ...
      >
        ...
        {items.length > 0 && (
          <span style={STORE_LABEL_STYLE}>
            {count} sản phẩm · {items.length} dòng
          </span>
        )}
      </section>
          ...
          {session && pendingGuestCount > 0 && (
            ...
            >
              ...
              <button
                ...
                disabled={pendingMergeBusy}
                onClick={() => void run('guest-cart-merge', mergePending)}
              >
                {pendingMergeBusy ? 'Đang thử lại…' : 'Thử lại'}
              </button>
            </p>
          )}
          <section
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column', borderRight: '2px solid var(--color-text)' }}>
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', border: '2px solid var(--color-text)' }}>
                        <button
                          type="button"
                          aria-label={`Bớt một ${item.productName}`}
                          disabled={busy || item.quantity <= 1}
                          onClick={() => void run(item.id, () => update(item.id, item.quantity - 1))}
                          style={stepStyle(busy || item.quantity <= 1)}
                        >
                          −
                        </button>
                        <span style={{ minWidth: 36, textAlign: 'center', fontSize: 14, fontWeight: 600 }}>
                          {item.quantity}
                        </span>
                        <button
                          type="button"
                          aria-label={`Thêm một ${item.productName}`}
                          disabled={busy || item.quantity >= MAX_QTY}
                          onClick={() => void run(item.id, () => update(item.id, item.quantity + 1))}
                          style={stepStyle(busy || item.quantity >= MAX_QTY)}
                        >
                          +
                        </button>
                      </div>
              ...
              >
                ...
                <button
                  ...
                  style={{ cursor: clearing ? 'not-allowed' : 'pointer' }}
                >
            ...
            >
              ...
              >
                ...
                <span
                  style={{ fontFamily: 'var(--font-heading)', fontWeight: 800, fontSize: 26, letterSpacing: '-.03em' }}
                >
              ...
              {session ? (
                ...
                <Link to="/auth" state={{ from: { pathname: '/thanh-toan' } }} className="btn btn-primary btn-block">
