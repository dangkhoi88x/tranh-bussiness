...
type PreviewCaption = { id?: string; text: string; x: number; y: number; fontSize: number; color: string; bold: boolean; align: 'left' | 'center' | 'right'; fontFamily: string };
type PreviewSlot = { imageId: string | null; zoom: number; panX: number; panY: number };
type PreviewSpread = { position: number; layoutCode: string; slots: PreviewSlot[]; captions?: PreviewCaption[]; backgroundColor?: string };
...
export function PhotobookSharePreviewPage() {
  ...
  useEffect(() => {
    ...
    apiRequest<SharePreviewData>(`/photobook-share-previews/${encodeURIComponent(token)}`)
      ...
      .catch(() => { if (alive) setError('Không tìm thấy bản xem trước hoặc link đã hết hạn.'); })
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100dvh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-body)' }}>
  ...
  if (error || !data) {
    return (
      <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 'var(--space-4)', fontFamily: 'var(--font-body)' }}>
        ...
        <Link to="/photobook" className="btn btn-primary" style={{ marginTop: 'var(--space-4)' }}>Tạo photobook của bạn</Link>
      </div>
    );
  }

  let spreads: PreviewSpread[] = [];
  try { spreads = JSON.parse(data.spreadsJson); } catch { /* empty */ }

  return (
    <div style={{ minHeight: '100dvh', fontFamily: 'var(--font-body)', background: 'var(--color-bg)' }}>
      {/* Header */}
      <header style={{
        display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 'var(--space-4) var(--space-6)',
        borderBottom: '2px solid var(--color-text)',
      }}>
        <Link to="/" style={{ textDecoration: 'none', color: 'var(--color-text)', fontFamily: 'var(--font-heading)', fontWeight: 800, fontSize: 18 }}>
          Bubble Memories
        </Link>
        <Link to="/photobook" className="btn btn-secondary" style={{ fontSize: 12 }}>Tạo photobook</Link>
      </header>

      {/* Content */}
      <main style={{ maxWidth: 1000, margin: '0 auto', padding: 'var(--space-8)' }}>
        <div style={{ textAlign: 'center', marginBottom: 'var(--space-8)' }}>
          <p style={{ margin: '0 0 var(--space-2)', fontSize: 11, letterSpacing: '.2em', textTransform: 'uppercase', color: 'var(--color-neutral-500)' }}>Bản xem trước</p>
          <h1 style={{ margin: 0, fontFamily: 'var(--font-heading)', fontWeight: 800, fontSize: 28, letterSpacing: '-.03em' }}>Photobook Preview</h1>
          {(data.sizeLabel || data.pageCount || data.finish) && (
            <p style={{ margin: 'var(--space-2) 0 0', fontSize: 13, color: 'var(--color-neutral-700)' }}>
              {[data.sizeLabel, data.pageCount ? `${data.pageCount} trang` : null, data.finish, `${spreads.length} spread`].filter(Boolean).join(' · ')}
            </p>
          )}
        </div>
        ...
        <div style={{ marginTop: 'var(--space-8)' }}>
          <h2 style={{ fontFamily: 'var(--font-heading)', fontWeight: 700, fontSize: 18, margin: '0 0 var(--space-4)' }}>Tất cả spread</h2>
            ...
            {spreads.map((spread, idx) => {
              ...
              return (
                <div key={idx} style={{
                  ...
                  background: 'var(--color-bg)', padding: 6,
                }}>
        ...
        {/* CTA */}
        <div style={{ textAlign: 'center', marginTop: 'var(--space-10)', padding: 'var(--space-8)', borderTop: '2px solid var(--color-text)' }}>
          <p style={{ margin: '0 0 var(--space-4)', fontSize: 14, color: 'var(--color-neutral-700)' }}>
            Bạn cũng muốn tạo một cuốn photobook?
          </p>
          <Link to="/photobook" className="btn btn-primary" style={{ fontSize: 14 }}>Tạo photobook của bạn →</Link>
        </div>

        {/* Expiry note */}
        <p style={{ textAlign: 'center', fontSize: 11, color: 'var(--color-neutral-500)', marginTop: 'var(--space-6)' }}>
...
function SlotImageUrl({ url, zoom, panX, panY }: { url: string; zoom: number; panX: number; panY: number }) {
  ...
  return <img src={url} alt="" draggable={false} loading="lazy" decoding="async" style={{
    width: '100%', height: '100%', objectFit: 'cover', display: 'block',
    objectPosition: `${50 + (panX ?? 0) * panRange}% ${50 + (panY ?? 0) * panRange}%`,
    transform: `scale(${z})`, transformOrigin: 'center',
  }} />;
}
        ...
        return (
          <div key={i} style={{
            ...
            {url ? (
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', fontSize: 18, color: 'var(--color-neutral-300)' }}>+</div>
            )}
          </div>
        );
      })}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'var(--space-6)' }}>
      <div style={{ perspective: 1800, width: '100%', maxWidth: 640, margin: '0 auto' }}>
        <div style={{ position: 'relative', width: '100%', aspectRatio: '1.05 / 0.7', transformStyle: 'preserve-3d' }}>
          ...
          <div style={{
            ...
            {isBack && (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', fontSize: 13, color: 'var(--color-neutral-500)', fontStyle: 'italic' }}>
                Bìa sau
              </div>
            )}
          </div>
          <div style={{
            ...
            backfaceVisibility: 'hidden', borderRadius: '2px 6px 6px 2px', overflow: 'hidden',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)' }}>
        <button type="button" className="btn btn-ghost" disabled={page < 0 || flipping} onClick={goPrev}>← Trước</button>
        <button type="button" onClick={() => setAutoPlay(!autoPlay)} style={chipStyle(autoPlay)}>
          {autoPlay ? 'Dừng' : 'Tự lật'}
        </button>
        <button type="button" className="btn btn-ghost" disabled={page >= total || flipping} onClick={goNext}>Sau →</button>
      </div>
