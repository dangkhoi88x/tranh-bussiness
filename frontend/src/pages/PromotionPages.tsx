  ...
  return (
    <header className="catalog-header">
      <div>
        ...
        <h2>Khuyến mãi</h2>
        <p>
          Tạo mã giảm giá, giới hạn lượt dùng và kiểm soát phạm vi áp dụng cho
          cửa hàng.
        </p>
      </div>
      {action}
...
export function PromotionsPage() {
  ...
  async function load() {
    ...
    try {
      setData(
        await apiRequest<Page<Promotion>>(`/promotions?${query.toString()}`),
      );
    } catch (error) {
        ...
        >
          ...
          <label>
            ...
            >
              ...
              {(
                ["DRAFT", "ACTIVE", "INACTIVE", "EXPIRED"] as PromotionStatus[]
              ).map((status) => (
                ...
                {data?.items.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td>
                      {discountCopy(item)}
                      <small>
                        Đơn tối thiểu {currency.format(item.minOrderAmount)}
                      </small>
                    </td>
                    <td>
                    ...
                    <td className="table-actions">
                      ...
                      {item.status === "ACTIVE" && (
                        <button
                          onClick={() => void updateStatus(item, "INACTIVE")}
                        >
                          Tắt
                        </button>
                      )}
                      <button onClick={() => setViewingUsages(item)}>
                        Lịch sử
                      </button>
                      <button
      ...
      {editing !== undefined && (
        <PromotionForm
          ...
          onSaved={(saved) => {
            ...
            setMessage(
              `Đã lưu mã ${saved.code}. Mã mới tạo ở trạng thái DRAFT, hãy bật khi sẵn sàng.`,
            );
            void load();
          }}
        />
...
function PromotionForm({
  ...
  const [form, setForm] = useState({
    ...
    endAt: asLocalInput(
      item?.endAt ?? new Date(Date.now() + 7 * 86_400_000).toISOString(),
    ),
  ...
  useEffect(() => {
    ...
    void apiRequest<Variant[]>(
      `/products/management/${scopeProductId}/variants`,
    )
      .then(setVariants)
      .catch((cause: unknown) => setError(apiError(cause)));
  ...
  function addScope() {
    ...
    if (
      !target ||
      scopes.some(
        (scope) => scope.type === scopeType && scope.targetId === target.id,
      )
    )
      return;
    setScopes((current) => [
  ...
  async function submit(event: FormEvent) {
    ...
    setError(null);
    try {
      const saved = await apiRequest<Promotion>(
        item ? `/promotions/${item.id}` : "/promotions",
        {
  ...
  return (
    <Modal
      title={item ? `Sửa mã ${item.code}` : "Tạo khuyến mãi"}
      onClose={onClose}
      wide
    >
      <form className="admin-form" onSubmit={(event) => void submit(event)}>
          ...
          <div className="scope-picker">
            ...
            {scopeType === "VARIANT" && (
              <select
                ...
                onChange={(event) => setScopeProductId(event.target.value)}
              >
        ...
        {item?.status === "ACTIVE" && (
          <p className="form-error">
            Mã đang ACTIVE không thể sửa. Hãy tắt mã trước, rồi lưu lại nếu mã
...
function PromotionUsageModal({
  ...
  async function load() {
    ...
    try {
      setData(
        await apiRequest<Page<PromotionUsage>>(
          `/promotions/${item.id}/usages?page=${page}&size=20`,
        ),
      );
    } catch (cause) {
      setError(apiError(cause));
  ...
  return (
    <Modal title={`Lịch sử dùng mã ${item.code}`} onClose={onClose} wide>
      <p className="usage-intro">
        Theo dõi các lượt giữ mã, dùng thành công, hoàn trả hoặc hết hạn. Số
        tiền là mức giảm thực tế trên đơn.
      </p>
      {error && <p className="form-error">{error}</p>}
              ...
              {data?.items.map((usage) => (
                <tr key={usage.id}>
                  <td>
                    ...
                    <small>
                      {usage.orderId ? `#${usage.orderId.slice(0, 8)}` : "—"}
                    </small>
                  </td>
                  <td>
                  ...
                  <td>
                    <span
                      className={`status status--${usage.status.toLowerCase()}`}
                    >
                      {usage.status}
                    </span>
                  </td>
                  <td>
                    ...
                    {usage.consumedAt && (
                      <small>Dùng: {formatTime(usage.consumedAt)}</small>
                    )}
                    {usage.releasedAt && (
                      <small>Trả: {formatTime(usage.releasedAt)}</small>
                    )}
                    {usage.expiresAt &&
                      !usage.consumedAt &&
                      !usage.releasedAt && (
                ...
                <tr>
                  <td colSpan={6}>
                    <p className="empty-state">
                      Mã này chưa có lịch sử sử dụng.
                    </p>
                  </td>
                </tr>
              )}
