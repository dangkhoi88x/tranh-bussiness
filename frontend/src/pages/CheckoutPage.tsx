...
import { cartItemOptions, STORE_LABEL_STYLE, StoreNotice, StoreShell, StoreSummaryRow } from '../components/StoreShell';
...
export function CheckoutPage() {
  ...
  async function loadAddresses() {
    ...
    try {
      ...
      setAddressError(e instanceof ApiRequestError ? e.message : 'Không tải được sổ địa chỉ. Vui lòng thử lại.');
    } finally {
  ...
  const couponNeedsPreview =
    coupon.trim().length > 0 && promotionPreview?.couponCode.toUpperCase() !== coupon.trim().toUpperCase();
      ...
      <StoreShell cartCount={0}>
        ...
        >
          ...
          <p style={{ margin: 0, maxWidth: '46ch', fontSize: 15, lineHeight: 1.6, color: 'var(--color-neutral-800)' }}>
            ...
            <br />
            Xưởng sẽ xác nhận và liên hệ giao hàng sớm nhất.
          </p>
          ...
          >
            <Link to="/danh-muc/tranh-canvas" className="btn btn-secondary">
              Tiếp tục mua sắm
            </Link>
            <Link to={`/don-hang-cua-toi/${order.id}`} className="btn btn-primary">
              Theo dõi đơn hàng
            </Link>
    ...
    <StoreShell cartCount={count}>
      ...
      {!session ? (
        ...
        <StoreNotice
          ...
          body={addressError}
          action={
            <button type="button" className="btn btn-primary" onClick={() => void loadAddresses()}>
              Thử lại
            </button>
          }
        />
        ...
        <section
          ...
          style={{
            display: 'grid',
            gridTemplateColumns: 'minmax(0, 1fr) minmax(0, .58fr)',
            alignItems: 'start',
            borderTop: '2px solid var(--color-text)',
          }}
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', borderRight: '2px solid var(--color-text)' }}>
                    ...
                    >
                      ...
                      <span style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 0 }}>
                        <span style={{ fontWeight: 600, fontSize: 14 }}>
                          ...
                          {a.defaultAddress && (
                            <span style={{ ...STORE_LABEL_STYLE, marginLeft: 8, color: 'var(--color-accent)' }}>
                              Mặc định
                            </span>
                          )}
                        </span>
            ...
            <div style={{ padding: 'var(--space-4) var(--space-8)' }}>
              <Link to="/gio-hang" style={{ fontSize: 13, color: 'var(--color-accent)' }}>
                ← Sửa giỏ hàng
              </Link>
            </div>
          </div>
          ...
          <aside style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)', padding: 'var(--space-8)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', borderTop: '2px solid var(--color-text)' }}>
              ...
              {promotionPreview && promotionPreview.discountAmount > 0 && (
                <StoreSummaryRow
                  label={`Giảm giá · ${promotionPreview.couponCode}`}
                  value={`− ${formatPrice(promotionPreview.discountAmount)}`}
                  valueTone="discount"
                />
              )}
            ...
            {couponError && (
              <p role="alert" style={{ margin: '-10px 0 0', fontSize: 12, color: 'var(--color-accent-700)' }}>
                {couponError}
              </p>
            )}
            {promotionPreview && (
...
function Breadcrumb({ current }: { current: string }) {
  return (
    ...
    >
      ...
      <Link to="/gio-hang" style={{ color: 'var(--color-neutral-700)', textDecoration: 'none' }}>
        Giỏ hàng
      </Link>
      <span aria-hidden="true">/</span>
