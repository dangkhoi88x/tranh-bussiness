            ...
            {items.map((item) => (
              <tr key={item.id}>
                <td>
                  ...
                  <small>
                    {item.code}
                    {item.defaultTemplate ? ' · mặc định' : ''}
                  </small>
                </td>
                <td>
                  {item.layoutCycle.length} bố cục
                  <small>
                    {item.layoutCycle.slice(0, 4).join(' · ')}
                    {item.layoutCycle.length > 4 ? '…' : ''}
                  </small>
                </td>
                <td>
                ...
                <td>
                  <span className={`status status--${item.active ? 'active' : 'archived'}`}>
...
function TemplateForm({
  ...
  async function submit(e: FormEvent) {
    e.preventDefault();
    if (layoutCycle.length === 0) {
      setError('Chọn ít nhất một bố cục cho chu kỳ.');
      return;
    }
    if (spreadColors.length === 0) {
      setError('Cần ít nhất một màu nền.');
      return;
    }
    setBusy(true);
    setError(null);
    ...
    <Modal title={item ? `Sửa chủ đề ${item.name}` : 'Thêm chủ đề'} onClose={onClose} wide>
      <form className="admin-form" onSubmit={(e) => void submit(e)}>
        ...
        <fieldset>
          ...
          <p className="form-hint">Thứ tự này là thứ tự spread khi xưởng dựng cuốn; hết chu kỳ thì quay lại từ đầu.</p>
          <ol className="template-cycle">
            {layoutCycle.map((entry, index) => (
              <li key={index}>
                ...
                <select
                  value={entry}
                  onChange={(e) => setLayoutCycle((prev) => prev.map((c, i) => (i === index ? e.target.value : c)))}
                >
                  {SPREAD_LAYOUTS.map((layout) => (
                    <option key={layout.code} value={layout.code}>
                      {layout.name} ({layout.slots.length} ô)
                    </option>
                  ))}
                </select>
                <button
                  ...
                  aria-label={`Xoá vị trí ${index + 1}`}
                  onClick={() => setLayoutCycle((prev) => prev.filter((_, i) => i !== index))}
                >
                  ×
                </button>
              </li>
          ...
          <button
            ...
            onClick={() => setLayoutCycle((prev) => [...prev, SPREAD_LAYOUTS[0].code])}
          >
            + Thêm bố cục
          </button>
        </fieldset>
        ...
        <fieldset>
          ...
          <div className="template-colors">
            {spreadColors.map((color, index) => (
              <span key={index}>
                ...
                <button
                  ...
                  onClick={() => setSpreadColors((prev) => prev.filter((_, i) => i !== index))}
                >
                  ×
                </button>
              </span>
            ...
            <button
              type="button"
              className="ghost-button"
              onClick={() => setSpreadColors((prev) => [...prev, '#ffffff'])}
            >
              + Thêm màu
            </button>
          </div>
        ...
        <fieldset>
            ...
            <div className="form-grid" key={index}>
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) => editCaption(index, { fontFamily: e.target.value })}
                  required
                />
              </label>
              <label>
                ...
                <input
                  type="color"
                  value={caption.color}
                  onChange={(e) => editCaption(index, { color: e.target.value })}
                />
              </label>
              <label>
                Canh
                <select
                  value={caption.align}
                  onChange={(e) => editCaption(index, { align: e.target.value as PresetCaption['align'] })}
                >
              ...
              <button
                type="button"
                className="ghost-button"
                onClick={() => setPresetCaptions((prev) => prev.filter((_, i) => i !== index))}
              >
                Xoá caption
              </button>
            </div>
          ...
          <button
            ...
            className="ghost-button"
            onClick={() =>
              setPresetCaptions((prev) => [
                ...prev,
                { ...EMPTY_CAPTION, fontFamily: defaultFont, color: defaultCaptionColor },
              ])
            }
          >
            + Thêm caption
        ...
        <p className="form-hint">
          Chủ đề mặc định là chu kỳ áp dụng cho cuốn khách mua mà không chọn chủ đề nào, nên không ẩn được và luôn phải
          có đúng một chủ đề giữ vai trò này.
        </p>
