...
function Breadcrumb() {
  return (
    ...
    >
      ...
      <span style={{ color: 'var(--color-text)' }}>Tài khoản</span>
    </nav>
  );
}
...
export function AccountPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [profileResult, addressResult] = await Promise.allSettled([fetchCurrentUser(), fetchShippingAddresses()]);
    if (profileResult.status === 'fulfilled') setProfile(profileResult.value);
    else
      setProfileError(profileResult.reason instanceof Error ? profileResult.reason.message : 'Không tải được hồ sơ.');
    if (addressResult.status === 'fulfilled') setAddresses(addressResult.value);
    else
      setAddressError(
        addressResult.reason instanceof Error ? addressResult.reason.message : 'Không tải được sổ địa chỉ.',
      );
    setLoading(false);
  }, []);
  ...
  async function saveAddress() {
    ...
    try {
      const saved =
        editing === 'new' ? await createShippingAddress(form) : await updateShippingAddress(editing!.id, form);
  ...
  async function submitPasswordChange() {
    ...
    if (
      ...
      passwordForm.newPassword.length > MAX_PASSWORD_LENGTH
    ) {
  ...
  const displayRole = profile?.roles.includes('ADMIN')
    ? 'Quản trị viên'
  ...
  return (
    <StoreShell cartCount={count}>
      ...
      >
        <div>
          ...
          <p style={{ margin: 'var(--space-2) 0 0', fontSize: 14, color: 'var(--color-neutral-800)' }}>
            Hồ sơ, địa chỉ giao hàng và đơn đã đặt của bạn.
          </p>
        </div>
      ...
      {loading ? (
        <StoreNotice title="Đang tải tài khoản…" body="" />
      ) : (
        <section
          data-split=""
          style={{
            display: 'grid',
            gridTemplateColumns: 'minmax(0, .76fr) minmax(320px, 1.24fr)',
            alignItems: 'start',
          }}
        >
          <aside
          ...
          >
            <section>
              ...
              >
                ...
                {profile && !editingProfile && (
                  <button type="button" className="btn btn-ghost" onClick={startEditingProfile}>
                    Sửa
                  </button>
                )}
              </div>
              {profileError ? (
                ...
                >
                  <div style={fieldGroup}>
                    ...
                    <label style={fieldLabel}>
                      Số điện thoại{' '}
                      <em style={{ fontStyle: 'normal', fontWeight: 400, color: 'var(--color-neutral-800)' }}>
                        (không bắt buộc)
                      </em>
                      <input
                        style={fieldInput}
                    ...
                    <p style={{ margin: 0, fontSize: 12, color: 'var(--color-neutral-800)', lineHeight: 1.5 }}>
                      Email đăng nhập <strong style={{ overflowWrap: 'anywhere' }}>{email}</strong> không thay đổi được
                      tại đây.
                    </p>
                  </div>
                  {profileFormError && (
                    <p role="alert" style={noticeError}>
                      {profileFormError}
                    </p>
                  )}
                  <div
                    style={{ display: 'flex', gap: 'var(--space-2)', marginTop: 'var(--space-4)', flexWrap: 'wrap' }}
                  >
                ...
                <div style={block}>
                  ...
                  {profile?.phone && (
                    <p style={{ margin: '4px 0 0', fontSize: 13, color: 'var(--color-neutral-800)' }}>
                      {profile.phone}
                    </p>
                  )}
                  <span style={{ display: 'inline-block', marginTop: 'var(--space-3)', ...STORE_LABEL_STYLE }}>
                    {displayRole}
                  </span>
                  {profileSaved && (
                    <p role="status" style={noticeSuccess}>
                      Đã cập nhật hồ sơ.
                    </p>
                  )}
                </div>
              ...
              {changingPassword ? (
                ...
                >
                  <div style={fieldGroup}>
                    ...
                    <p style={{ margin: 0, fontSize: 12, color: 'var(--color-neutral-800)', lineHeight: 1.5 }}>
                      Tối thiểu {MIN_PASSWORD_LENGTH} ký tự. Đổi xong, mọi thiết bị khác đang đăng nhập sẽ bị đăng xuất.
                    </p>
                  </div>
                  {passwordError && (
                    <p role="alert" style={noticeError}>
                ...
                <div style={block}>
                  <p style={{ margin: 0, fontSize: 13, lineHeight: 1.55, color: 'var(--color-neutral-800)' }}>
                    Đặt mật khẩu mới cho tài khoản. Bạn sẽ cần nhập mật khẩu hiện tại.
                  </p>
                  {passwordChanged && (
                    <p role="status" style={noticeSuccess}>
            ...
            <section>
              ...
              <Link to="/don-hang-cua-toi" className="btn btn-primary btn-block">
                Đơn hàng của tôi →
              </Link>
              <p
                style={{
              ...
              >
                Xem tiến trình xưởng xác nhận, giao hàng và thanh toán.
              </p>
            </section>
          </aside>
          ...
          <section style={{ minWidth: 0, padding: 'var(--space-6) var(--space-8)' }}>
            ...
            >
              <div>
                <h2 style={{ ...sectionTitle, marginBottom: 0 }}>Sổ địa chỉ</h2>
                <p style={{ margin: 'var(--space-2) 0 0', fontSize: 13, color: 'var(--color-neutral-800)' }}>
                  Địa chỉ này sẽ có sẵn khi bạn thanh toán.
                </p>
              </div>
              {!editing && (
                <button type="button" className="btn btn-primary" onClick={startNewAddress}>
                  + Thêm địa chỉ
                </button>
              )}
            </div>
            {addressError && (
              <p role="alert" style={{ margin: '0 0 var(--space-4)', fontSize: 13, color: 'var(--color-accent-700)' }}>
                {addressError}
              </p>
            )}
            {editing && (
              ...
              >
                <h3 style={{ margin: '0 0 var(--space-4)', fontSize: 18 }}>
                  {editing === 'new' ? 'Thêm địa chỉ giao hàng' : 'Chỉnh sửa địa chỉ'}
                </h3>
                <ShippingAddressForm
                  form={form}
                  setField={setField}
                  ...
                  error={formError}
                  saving={saving}
                  submitLabel={editing === 'new' ? 'Thêm địa chỉ' : 'Cập nhật địa chỉ'}
                  onSave={() => void saveAddress()}
                  onCancel={() => setEditing(null)}
                />
              </section>
            )}
            {!editing && addresses.length === 0 && !addressError && (
              <StoreNotice
                title="Chưa có địa chỉ giao hàng"
                body="Thêm địa chỉ trước để thanh toán nhanh hơn ở lần đặt tranh tới."
                action={
                  <button type="button" className="btn btn-primary" onClick={startNewAddress}>
                    Thêm địa chỉ
                  </button>
                ...
                >
                  <div style={{ minWidth: 0 }}>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-2)', alignItems: 'baseline' }}>
                      <strong style={{ fontSize: 15 }}>{address.recipientName}</strong>
                      <span style={{ fontSize: 13, color: 'var(--color-neutral-800)' }}>{address.phone}</span>
                      {address.defaultAddress && (
                        <span style={{ ...STORE_LABEL_STYLE, color: 'var(--color-accent)' }}>Mặc định</span>
                      )}
                    </div>
                    <p
                      style={{ margin: 'var(--space-2) 0 0', fontSize: 13, lineHeight: 1.55, overflowWrap: 'anywhere' }}
                    >
                      {oneLine(address)}
                    </p>
                    {deleteCandidate === address.id && (
                      ...
                      >
                        <p style={{ margin: '0 0 var(--space-2)', fontSize: 12 }}>Xoá địa chỉ này khỏi sổ địa chỉ?</p>
                        <div style={{ display: 'flex', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
                          ...
                          >
                            {deletingId === address.id ? 'Đang xoá…' : 'Xác nhận xoá'}
                          </button>
                          <button
                            type="button"
                            className="btn btn-secondary"
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() => setDeleteCandidate(address.id)}
...
const noticeError: React.CSSProperties = {
  margin: 'var(--space-3) 0 0',
  fontSize: 13,
  lineHeight: 1.5,
