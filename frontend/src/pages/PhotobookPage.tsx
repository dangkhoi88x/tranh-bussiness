  ...
  return (
    <StoreShell cartCount={count}>
      ...
      >
        ...
        <div
          style={{
            position: 'absolute',
            inset: 0,
            background: 'linear-gradient(to bottom, rgba(0,0,0,.45) 0%, rgba(0,0,0,.15) 50%, rgba(0,0,0,.5) 100%)',
          }}
        />
        <div
          style={{
        ...
        >
          ...
          >
            Giữ khoảnh khắc trong từng trang giấy
          </h1>
          <p
            style={{
              margin: 0,
      ...
      {(catError || error) && (
        ...
        >
          Không tải được danh sách template. Vui lòng thử lại sau.
        </div>
      )}
      ...
      {!catError && !error && !loading && categories && !photobookCat && (
        ...
        >
          Chưa có template photobook nào.
        </div>
      )}
      ...
      <section style={{ position: 'relative', overflow: 'hidden', borderBottom: '2px solid var(--color-text)' }}>
        <div style={{ position: 'absolute', inset: 0 }}>
          <Frame
            src={items[2]?.primaryImageUrl ?? items[0]?.primaryImageUrl ?? undefined}
        ...
        <div
          style={{
            position: 'absolute',
            inset: 0,
            background: 'linear-gradient(to bottom, rgba(32,30,29,.7), rgba(32,30,29,.85))',
          }}
        />
        <div
          style={{
        ...
        >
          <span
            style={{ fontSize: 11, letterSpacing: '.2em', textTransform: 'uppercase', color: 'rgba(255,255,255,.7)' }}
          >
            Chi tiết
          </span>
          <h2
          ...
          <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
            {TABS.map((t, k) => (
              ...
              >
                <span style={{ fontSize: 10, marginRight: 6, opacity: 0.6 }}>0{k + 1}.</span>
                {t}
              </button>
            ))}
          </div>
      ...
      >
        ...
        >
          ...
          >
            Muốn cuốn theo phong cách riêng?
          </h2>
...
function TemplateSection({ book, index }: { book: Product; index: number }) {
  ...
  const imageBlock = (
    <div
      ...
      onPointerEnter={(event) => {
        if (event.pointerType === 'mouse') setShowPreview(true);
      }}
      onPointerLeave={(event) => {
        if (event.pointerType === 'mouse') setShowPreview(false);
      }}
      ...
      style={{
        position: 'relative',
        overflow: 'hidden',
        borderRight: even ? '2px solid var(--color-text)' : undefined,
        borderLeft: even ? undefined : '2px solid var(--color-text)',
      }}
    >
  ...
  const textBlock = (
    ...
    >
      ...
      >
        <span
          style={{ fontSize: 11, letterSpacing: '.2em', textTransform: 'uppercase', color: 'var(--color-neutral-700)' }}
        >
          Từ
        </span>
        <span style={{ fontFamily: 'var(--font-heading)', fontWeight: 800, fontSize: 24 }}>
          {formatPrice(displayPrice)}
        </span>
      </div>
      <div style={{ display: 'flex', gap: 'var(--space-3)', marginTop: 'var(--space-2)' }}>
        <Link to={`/photobook/${book.slug}`} className="btn btn-primary">
          Xem chi tiết
        </Link>
        <Link to="/dat-in" className="btn btn-secondary">
          Đặt cuốn này
        </Link>
      </div>
    </div>
  ...
  return (
    <section
      ...
      style={{
        display: 'grid',
        gridTemplateColumns: even ? 'minmax(0, 1.15fr) minmax(0, 1fr)' : 'minmax(0, 1fr) minmax(0, 1.15fr)',
        borderBottom: '2px solid var(--color-text)',
      }}
    >
      {even ? (
...
function TemplateQuickPreview({
  ...
  return (
    <div
      aria-label={`Xem trước spread mẫu của ${book.name}`}
      style={{
        position: 'absolute',
        inset: 0,
        zIndex: 1,
...
const PB_FAQS = [
  {
    q: 'Template là gì, tôi có thể tuỳ chỉnh không?',
    a: 'Template là bố cục sẵn với phông chữ, bảng màu và cách sắp ảnh đã được thiết kế. Bạn gửi ảnh, xưởng đặt vào template và gửi bản mềm duyệt. Nếu muốn đổi phông, thay màu hoặc thêm trang thì báo khi duyệt — sửa miễn phí 2 lần.',
  },
  {
    q: 'Cần gửi bao nhiêu ảnh?',
    a: 'Tuỳ bạn làm cách nào. Nếu để xưởng bố cục: cuốn 20 trang cần khoảng 60–80 ảnh, 40 trang cần 120–160 ảnh — gửi dư cũng được, xưởng sẽ chọn lọc giúp. Nếu bạn tự thiết kế trên web: chỉ cần đúng số ô ảnh của chủ đề bạn chọn, con số này hiện ngay ở bước chọn quy cách và thường ít hơn khoảng trên vì mỗi tấm được đặt vào một ô cố định.',
  },
  {
    q: 'Chất liệu giấy và bìa thế nào?',
    a: 'Ruột in trên giấy mỹ thuật 250gsm, phủ mờ chống loá. Bìa cứng bọc vải, ép nhũ tên hoặc in UV tuỳ template. Có thể nâng cấp giấy ảnh 300gsm hoặc bìa da nếu cần.',
  },
  {
    q: 'Bao lâu thì nhận được?',
    a: 'Sau khi duyệt bản mềm: 7–10 ngày làm việc. Đơn gấp rút còn 5 ngày, phụ thu 30%. Ship toàn quốc, đóng hộp chống ẩm.',
  },
  {
    q: 'Đặt số lượng nhiều có giảm giá không?',
    a: 'Từ 5 cuốn trở lên có giá sỉ. Liên hệ xưởng để nhận bảng giá riêng theo số lượng và quy cách.',
  },
];
          ...
          {PB_FAQS.map((f, k) => {
            ...
            return (
              <div key={f.q} style={{ borderBottom: '2px solid var(--color-divider)' }}>
                ...
                >
                  <span style={{ fontSize: 11, letterSpacing: '.18em', color: 'var(--color-neutral-700)' }}>
                    0{k + 1}
                  </span>
                  <span
                    style={{
