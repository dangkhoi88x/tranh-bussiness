...
- Stock is reserved with a **conditional `UPDATE`** (`ProductRepository.decreaseStock` / `ProductVariantRepository.decreaseStock`), inside the same checkout transaction that creates the order: the check and the decrement are one statement, so two concurrent checkouts cannot both see the last unit. Reading stock and then writing `stock - quantity` oversells even behind `SELECT ... FOR UPDATE`, because the entity is already in the persistence context and Hibernate hands back the stale quantity. Cancellation restores through the matching `increaseStock`.
